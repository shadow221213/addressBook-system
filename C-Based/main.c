...
// 排序通讯录
void sortPerson(struct Person* begin, int size) {
    for( int i = 0; i < size; i++ )
        for( int j = i + 1; j < size; j++ )
            if( begin[i].Number > begin[j].Number ) {
                struct Person tmp = begin[i];
                begin[i] = begin[j];
                begin[j] = tmp;
            }
}
...
// 查找生日
void FindBirth(struct Birthday birth) {
    int cnt = false;
    for( int i = 0; i < Abs.Size; i++ )
        if( Abs.Array[i].Birth.month == birth.month &&
            Abs.Array[i].Birth.day == birth.day ) {
